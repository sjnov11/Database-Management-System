...
void load_page(int table_id, off_t offset, Page* page) {
    ...
    char pathname[10] = "DATA";
    char c_tid[10];
    sprintf(c_tid, "%d", table_id);
    strcat(pathname, c_tid);

    int tablefile = open(pathname, O_RDWR);
	lseek(tablefile, offset, SEEK_SET);
    read(tablefile, page, PAGE_SIZE);
...
void flush_page(int table_id, Page* page) {
    ...
    int tablefile = open(pathname, O_RDWR);
	lseek(tablefile, page->file_offset, SEEK_SET);
    write(tablefile, page, PAGE_SIZE);
    
    close(tablefile);

}
int64_t get_page_lsn(Page* page) {
    return *((int64_t*)((char*)page + 24));
}

void set_page_lsn(Page* page, int64_t* lsn) {
    memcpy( ((char*)page) + 24, lsn, sizeof(int64_t));
